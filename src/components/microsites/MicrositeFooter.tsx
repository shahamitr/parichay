...
export default function MicrositeFooter({ brand, branch }: MicrositeFooterProps) {
  const socialMedia = branch.socialMedia as any;
    ...
    try {
      await fetch('/api/analytics/track', {
        ...
        body: JSON.stringify({
          ...
          branchId: branch.id,
          brandId: brand.id,
          metadata: { action: 'social_media', platform, url, section: 'footer' },
        }),
      });
    } catch (error) {
  ...
  const hasSocialMedia = socialMedia && (
    ...
    socialMedia.twitter || socialMedia.youtube || socialMedia.whatsapp
  );
