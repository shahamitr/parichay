...
export default function AdvancedSearch({
  ...
  return (
    <div ref={searchRef} className={cn('relative', className)}>
      ...
      <div className="relative flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-600 rounded-xl focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-transparent outline-none transition-all"
          />
          {query && (
            <button
              onClick={() => setQuery('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded-full transition-colors"
            >
        ...
        >
          ...
          {filters.length > 0 && (
            <span className="px-2 py-0.5 bg-primary-500 dark:bg-primary-600 text-white text-xs rounded-full">
              {filters.length}
            </span>
          )}
        </button>
      ...
      {filters.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-3">
          {filters.map((filter, index) => (
            <div
              key={index}
              className="flex items-center gap-2 px-3 py-1.5 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 rounded-lg text-sm"
            >
          ...
          <button
            ...
            className="px-3 py-1.5 text-sm text-neutral-600 dark:text-neutral-400 hover:text-error-600 dark:hover:text-error-400 transition-colors"
          >
            Clear all
          </button>
        </div>
      ...
      {/* Filter Panel */}
      {showFilters && (
        <div className="absolute top-full left-0 right-0 mt-2 p-4 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl shadow-xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-neutral-900 dark:text-white">Add Filters</h3>
            <button
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ...
                      : 'bg-neutral-50 dark:bg-neutral-700/50 text-neutral-700 dark:text-neutral-300 hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-700 dark:hover:text-primary-300'
                  )}
                >
          ...
          {filters.map((filter, index) => {
            ...
            return (
              <div key={index} className="mt-4 p-3 bg-neutral-50 dark:bg-neutral-700/50 rounded-lg">
                ...
                {option.type === 'select' && option.options ? (
                  <select
                    ...
                    className="w-full px-3 py-2 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-600 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 outline-none"
                  >
